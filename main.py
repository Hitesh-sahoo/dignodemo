...
app = FastAPI(title="🏥Diagnoverse_ai")
...
def home():
    return {"message": "Welcome to 🏥Diagnoverse_ai . Connect via Streamlit."}
...
def chatbot(symptoms: str = Query(...), patient_id: int = Query(...)):
    ...
    if any(greet in symptoms.lower() for greet in greetings):
        conn.close()
        return {"message": "👋 Hello! I'm your AI Medical Assistant. Please describe your symptoms,I’ll help you find the right doctor."}
    ...
    try:
        ...
        if not free_slots:
            conn.close()
            return{"message": f"⚠️ {doctor['name']} is fully booked today. Please try another doctor."}
        ...
        return {
            "message": (
                f"🩺Based on your symptoms, I recommend consulting a **{specialization}**.\n"
                f"👨‍⚕️ Doctor: **{doctor['name']}**\n"
                f"🕒Available Slot: **{slots_text}**\n\n"
                f"👉 Shall I book the earliest slot ({selected_slot})? (Yes/No)"
